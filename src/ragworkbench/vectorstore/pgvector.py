"""ragworkbench/vectorstore/pgvector -- Postgres + pgvector-backed VectorStore.
...
- ``filter`` is rendered into the WHERE clause as jsonb predicates on ``metadata``
  ...
  :func:`ragworkbench.ingest.filters.matches_filter` (scalar equality + ``$gte/$gt/
...
_INSTALL_HINT = "pip install 'ragworkbench[vectorstore-pgvector]'"
...
class PgvectorStore:
    ...
    def _render_filter(filter: dict | None) -> tuple[str, list]:
        ...
        Mirrors :func:`ragworkbench.ingest.filters.matches_filter`: scalar -> equality
