"""ragworkbench/vectorstore -- pluggable VectorStore backends + ``build_store`` factory.
...
def build_store(conf: dict[str, Any], embedder: EmbeddingClient | None = None):
    ...
    if backend == "faiss":
        from ragworkbench.vectorstore.faiss import FaissVectorStore

        return FaissVectorStore(embedder=embedder, dim=conf.get("dim"))
    if backend == "chroma":
        from ragworkbench.vectorstore.chroma import ChromaVectorStore

        return ChromaVectorStore(embedder=embedder, collection=conf.get("collection", "rwb"), path=conf.get("path"))
    if backend == "pgvector":
        from ragworkbench.vectorstore.pgvector import PgvectorStore

        return PgvectorStore(embedder=embedder, dsn=conf.get("dsn"), table=conf.get("table", "rwb_chunks"))
