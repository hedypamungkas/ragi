"""ragworkbench/retrieval/retriever -- lexical + embedding-based retrievers.
...
self-register on import via :func:`ragworkbench.registry.register_retriever` so a
plain ``from ragworkbench.retrieval.retriever import ...`` is enough to populate
...
Semantic/Hybrid need an :class:`ragworkbench.protocols.EmbeddingClient` (e.g.
:class:`ragworkbench.embeddings.openai.OpenAIEmbeddingClient`); they degrade to
keyword retrieval when no embedder is supplied or the endpoint returns ``None``,
and reuse the process-level :data:`ragworkbench.embeddings.cache._EMBEDDING_CACHE`
...
from ragworkbench.embeddings.cache import _EMBEDDING_CACHE
from ragworkbench.ingest.filters import matches_filter
from ragworkbench.registry import register_retriever, retriever_registry
from ragworkbench.retrieval.stopwords import STOPWORDS_EN, STOPWORDS_ID, get_stopwords
    ...
    if stemmer == "id":
        try:
            ...
            _logger.warning(
                ...
                "pip install 'ragworkbench[indo-nlp]'. Skipping stemming."
            )
            return None
...
class BaseRetriever(ABC):
    ...
    filter (NOT an ACL boundary -- see :mod:`ragworkbench.ingest.filters`).
    """
    ...
    def _allowed_ids(self, metadata_filter: dict | None) -> set[str] | None:
        ...
        tenants apart; see :mod:`ragworkbench.ingest.filters`.
        """
...
class SemanticRetriever(BaseRetriever):
    ...
    ``embedder`` satisfies :class:`ragworkbench.protocols.EmbeddingClient`
