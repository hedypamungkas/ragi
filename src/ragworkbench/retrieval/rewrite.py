"""ragworkbench/retrieval/rewrite -- query rewriting + HyDE as a measurable Retriever stage.
...
- :class:`QueryRewriter` -- rule-based normalization (always-on, deterministic,
  ...
  any failure.
- :class:`RewritingRetriever` -- a :class:`~ragworkbench.retrieval.retriever.BaseRetriever`
...
if TYPE_CHECKING:
    from ragworkbench.protocols import ChatClient
    from ragworkbench.types import RetrievalResult
...
class QueryRewriter:
    ...
    ``client`` follows the :class:`~ragworkbench.protocols.ChatClient` protocol
...
class RewritingRetriever(BaseRetriever):
    """:class:`~ragworkbench.retrieval.retriever.BaseRetriever` wrapper that rewrites first.
...
class MockChatClient:
    """Scripted or transform-based :class:`~ragworkbench.protocols.ChatClient` for tests.
