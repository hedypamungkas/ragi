"""ragworkbench/retrieval/augmentation -- context-injection strategies (agent seam).
...
``_maybe_rewrite`` step). In ragworkbench rewrite lives in a separate
...
:func:`ragworkbench.registry.register_augmentation` so a plain
``from ragworkbench.retrieval.augmentation import ...`` is enough to populate
...
from ragworkbench.registry import register_augmentation
