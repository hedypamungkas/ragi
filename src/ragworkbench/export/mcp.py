"""ragworkbench/export/mcp -- read-only MCP server (official ``mcp`` SDK / FastMCP).
    ...
    def __init__(
        ...
        *,
        ...
        try:
            ...
            raise LLMInvalidRequestError("(mcp) the mcp SDK is required: pip install 'ragworkbench[mcp]'") from e
...
def serve_from_config(
    ...
    *,
    ...
    -- ``ragwb serve`` does not wire one in v0.3, so they degrade to keyword over MCP;
    pass ``embedder=`` programmatically for full semantic serve.
    """
    import ragworkbench as rwb

    rwb.register_builtins()
    retriever = rwb.build_pipeline(conf, embedder=embedder)
