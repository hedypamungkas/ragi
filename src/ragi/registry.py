"""ragi/registry -- decorator-based component registry + build_pipeline().
...
if TYPE_CHECKING:
    ...
    from ragi.protocols import ChatClient, EmbeddingClient
...
def _load_documents(conf: dict[str, Any]) -> tuple[BaseChunker, list[Chunk]]:
    ...
    from ragi.ingest.parsers import dispatch_parser
    from ragi.ingest.sources import (
        DocumentCache,
...
def build_pipeline(
    ...
    *,
    ...
    if conf.get("retriever") == "vectorstore":
        ...
        from ragi.vectorstore import build_store
    ...
    if conf.get("query_rewrite") or conf.get("hyde"):
        from ragi.retrieval.rewrite import RewritingRetriever
    ...
    if isinstance(rerank_conf, dict):
        from ragi.retrieval.rerank import CrossEncoderReranker, build_rerank_client
