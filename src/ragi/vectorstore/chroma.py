"""ragi/vectorstore/chroma -- ChromaDB-backed VectorStore.
...
the same ``$``-operators as :func:`ragi.ingest.filters.matches_filter`, so ``filter``
...
_INSTALL_HINT = "pip install 'ragi[vectorstore-chroma]'"
