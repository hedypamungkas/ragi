"""ragi/adapters/llamaindex -- LlamaIndex ``BaseRetriever`` adapter.

Adapts a ragi :class:`Retriever` as a LlamaIndex
...
``pip install 'ragi[adapters-llamaindex]'`` when you adopt it. The import-gate itself
...
try:
    ...
    raise LLMInvalidRequestError(
        "(adapters-llamaindex) llama-index required: pip install 'ragi[adapters-llamaindex]'"
    ) from e
...
class LlamaIndexRetrieverAdapter(BaseRetriever):
    """A ragi Retriever exposed as a LlamaIndex BaseRetriever."""

    ragi_retriever: Any = None
    top_k: int = 5
