"""ragi/workbench/cli -- the ``ragi`` command: ingest | eval | compare | serve | export-tool-schema.

The closed-loop driver:

    ragi ingest <config>                          # validate config + index corpus + probe
    ragi eval <config> --dataset msmarco -n 120    # measure (Mode A, no API key)
    ragi compare <cfgA> <cfgB> --metric recall@10  # A/B iterate
    ragi serve <config> --transport stdio          # ship as a read-only MCP server
    ragi export-tool-schema --format openai        # ship as an OpenAI/Claude tool spec
"""
...
def _prepare_corpus(config: dict, dataset) -> tuple[dict, str | None]:
    ...
    if dataset.name == "synthetic":
        from ragi.eval.datasets import SYNTHETIC_CORPUS

        tmp = tempfile.mkdtemp(prefix="ragi_synth_")
...
def _build_retriever(config: dict, dataset):
    ...
    retriever = ragi.build_pipeline(cfg)
    if retriever is None:
        raise SystemExit(
            "ERROR: pipeline produced no retriever (disabled, or no documents loaded at the "
            "configured path). Check `documents:` / `corpus_dir` and run `ragi ingest`."
        )
    return retriever
...
def cmd_ingest(args: argparse.Namespace) -> int:
    ...
    ragi.register_builtins()
    print(f"chunkers: {ragi.chunker_registry.list_available()}")
    print(f"retrievers: {ragi.retriever_registry.list_available()}")
    print(f"parsers: {ragi.parser_registry.list_available()}")
    # Use the synthetic corpus for a probe if no documents configured (offline-safe).
    from ragi.eval.datasets import resolve_dataset
...
def cmd_eval(args: argparse.Namespace) -> int:
    from ragi.eval.datasets import resolve_dataset
    from ragi.eval.runner import StandaloneEvalRunner, format_report
...
def cmd_compare(args: argparse.Namespace) -> int:
    from ragi.eval.compare import compare, format_comparison
    from ragi.eval.datasets import resolve_dataset
...
def cmd_serve(args: argparse.Namespace) -> int:
    ...
    """
    from ragi.export.mcp import serve_from_config
